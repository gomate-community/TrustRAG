...
from trustrag.modules.engine.chroma import ChromaEngine
if __name__ == '__main__':
    ...
    chroma_engine = ChromaEngine(
        ...
        chroma_client_params={"uri": "http://localhost:8001"},
    )
    ...
    vectors = chroma_engine.embedding_generator.generate_embeddings([doc["description"] for doc in documents])
    print(vectors.shape)
    payload = [doc for doc in documents]

    # Upload vectors and payload
    chroma_engine.upload_vectors(vectors=vectors, payload=payload)


    # 过滤搜索示例
    conditions = [
        {"key": "city", "value": "Berlin"},
        {"key": "likes", "value": 50, "operator": ">"}
    ]
    filter_expr = chroma_engine.build_filter(conditions)
    results = chroma_engine.search(
